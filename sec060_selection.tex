...
$\HT=200\gev$ and extend in $50\gev$ steps up to $\HT=400\gev$. After this, the
bins are $100\gev$ wide extending up to the highest \HT reachable that still has
statistics in the control region. These bin widths have been rationalised with
...
increased. The production of heavy SUSY particles, such as gluinos, will result
in large values \HT . This is illustrated in the \HT bins that events from a pair
produced gluino model sit, Fig.~\ref{fig:sigYields}. To increase the sensitivity
...
relevant control samples, to allow background prediction. Enough events is
defined as the number that result in a statistical uncertainty that is
...
\begin{figure}[h!]
  ...
  \\
  \caption{\label{fig:ewkYields} Yields at $4\fbinv$ for the GJets MC 
