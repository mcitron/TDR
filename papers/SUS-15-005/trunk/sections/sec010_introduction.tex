...
%can alleviate the gauge hierarchy problem of the SM by cancelling
%radiative corrections to the Higgs mass from loop processes involving
%SM particles~\cite{ref:hierarchy1, ref:hierarchy2}. These corrections
%diverge quadratically with the highest scale up to which the SM is
%expected to be a valid theory, often taken to be a Planck scale of
%$2\times10^{19}\GeV$. Such a cancellation can only be accommodated in
%the SM through an extreme level of fine tuning of the SM fermion and
%boson masses. A ``natural'' solution, with minimal fine-tuning,
%requires the $\chiz_1$, $\PSQt_\text{L}$, $\PSQt_\text{R}$,
...
%tuning, imply the presence of states accessible by the CERN LHC.
