...
\begin{table*}[tb]
  ...
  \begin{tabular}{ ll }
    ...
    \hline
    \multicolumn{2}{l}{{\bf Control regions (CR)}, preselection +}                                                                                                           \\
...
categories in (\njet,\nb). FIXME. Need a comment on number of bins.

\begin{table}[tb]
  \topcaption{32 categories.}
  \label{tab:binning}
  \centering
  \footnotesize
  \begin{tabular}{ lcccc }
...
%  = \frac{1}{2} \frac{1 - (\dht/\scalht)}{\sqrt{1 - (\HTmiss/\scalht)^2}}\,.
\end{equation}
...
\begin{figure*}[tbhp]
  ...
  \caption{(Left) The \alphat distribution observed in data for events
    ...
    % The uncertainties in the SM expectation are dominated by the
    % statistical uncertainties associated with the limited sample of
...
\subsection{Using \HTmiss as a discriminating variable}
...
The \mj event sample is defined by the baseline preselection
