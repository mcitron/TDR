...
Each jet $j_\text{i}$ considered by this search is required to satisfy
$\Pt^{j_\text{i}} > 40\GeV$ and $|\eta^{j_\text{i}}| < 3$. The number
...
$\HTmiss = |\sum_{i=1}^{\njet} \ptvec^{\,j_\text{i}}|$, is used to
identify events with a significant imbalance in transverse
...
\Pt thresholds but an appreciable \Pt vector sum so as to contribute
...
\begin{table*}[tb]
  ...
  \begin{tabular}{ ll }
    ...
    \ETmiss cleaning             & Filters related to beam and instrumental effects, and reconstruction failures                                    \\ 
...
\Et^{\mathrm{j}_i}$, where $N_\text{jet}$ is the number of
...
in the average jet energy and a consequent (relative) improvement in
...
\begin{figure*}[!h]
  ...
  \caption{ (Left) The \alphat distribution observed in data for
    ...
    processes) are estimated using a \mj data control sample,
    described in Section~\ref{sec:ewk_background}. These distributions
    illustrate the performance of these two variables in a kinematic
    phase space that is representative of the signal region. The
    statistical uncertainties for the multijet and SM expectations are
    represented by the hatched areas (visible only for statistically
    limited bins). The final bin of each distribution contains the
    overflow events.  }
...
define the signal region, as defined in Table~\ref{tab:selections},
...
In the case of the \alphat distribution, the events that satisfy
$\alphat < 0.55$ must fulfil only the common preselection criteria
...
independent of the \alphat and \bdphi variables. The contribution from
...
descriptions found below and summarised in
...
used to estimate the multijet background in the signal region. Three
...
values, respectively, in the region of $\sim$90\% and $\sim$99\%.
