...
$\PSGczDo$, which is weakly interacting and massive. This SUSY particle
...
(higgsino-like) $\PSGczDo$ are at or near the electroweak
...
the $\PSQt$ and the $\PSGczDo$ are light and nearly degenerate in mass
...
uncertainties in the ratios $\mathcal{R}^\mathrm{QCD}(\njet, \scalht)$,
as described below.
...
$\mathcal{A}\,\varepsilon$ are considered. Each source of
...
$\PSGcz_0$ particles carry very little momentum.  The \texttt{T5ttcc}
...
expected and observed excluded $m_{\PSg}$ or $m_{\PSQ}$ and
$m_{\PSGczDo}$ masses for each class of simplified model.
