...
\abstract{A search for supersymmetry is performed in final states
  comprising one or more jets and an imbalance in transverse
  momentum. A sample of events resulting from pp collisions at a
  centre-of-mass energy of $13~\text{TeV}$ is analysed. The event data
  were recorded with the CMS detector at the CERN LHC in 2016 and
  correspond to an integrated luminosity of $35.9~\text{fb}^{-1}$.
  The search provides sensitivity to supersymmetric models that
  predict the pair production of heavy, colour-charged particles that
  are assumed to be meta-stable or undergo prompt decays to stable
  weakly interacting massive particles, such as the neutralino.  The
  search employs standard event reconstruction techniques, without the
  use of specialised algorithms to target displaced jets arising from
  the decays of long-lived states. Several kinematic variables are
  used to suppress the dominant standard model background, multijet
...
%  X and Y\GeV are also probed.
  ...
  with displaced jets and momentum imbalance. Broad coverage is
  realised for a range of gluino and neutralino masses and lifetimes
  ($\tau$). Gluino masses below X\TeV are excluded, irrespective of
  the mass hierarchy and lifetime (from prompt to stable and values
  inbetween). Gluino masses up to Y\TeV are probed for $\tau =
  10^{-2}\ns$.  }
...
hadronisation of light- or heavy-flavour quarks, accompanied by a
significant imbalance in transverse momentum \ptvecmiss.
...
data storage. The L1 and HLT algorithms used by this search are
summarised in Sec.~\ref{sec:selection}.

The particle-flow (PF) event algorithm~\cite{Sirunyan:2017ulk}
reconstructs and identifies candidate physics objects, including
...
corresponding associated missing transverse momentum. Charged-particle
tracks associated with vertices from additional proton-proton
interactions within the same or nearby bunch crossings
(pileup)~\cite{pileup} are not considered by the PF algorithm as part
of the global event reconstruction.

Candidate signal events, as well as samples of control data, are
defined by the absence or presence of photons and leptons that are
...
$\phi$ and $\eta$ represent pseudorapidity and the azimuthal angle.
...
the lepton \pt. The cone radius is dependent on the lepton \pt to
maintain high efficiency for semileptonic decays of Lorentz-boosted
top quarks~\cite{Rehermann:2010vq}: $\Delta R = \min [ \max( 0.05,
10\GeV / \pt ), 0.2 ]$. Isolated electrons and muons are required to
satisfy $\mathcal{I}_\text{rel} < 0.1$ and 0.2, respectively.
...
relative to the track \pt, satisfies $I^\text{track}_\text{rel} <
0.1$.
...
efficiency, which is the probability of correctly identifying jets as
originating from b quarks, as well as the ``mistag'' probability of
...
mass, a well measured dijet event with $\Et^{\text{j_1}} =
\Et^{\text{j_2}}$ and back-to-back jets ($\phi_{\rm j_1,j_2} = \pi$)
yields an \alphat value of 0.5. In the presence of a jet energy
mismeasurement, $\Et^{\text{j_1}} > \Et^{\text{j_2}}$ and $\alphat
