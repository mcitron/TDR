...
\abstract{A search for supersymmetry is performed in final states
  comprising one or more jets and missing transverse momentum from
  ...
  $35.9~\text{fb}^{-1}$.  The number of candidate signal events is
  ...
  metastable gluino scenario. Masses up to 1750\GeV and 900\GeV are
  probed for a proper decay length of 1\unit{mm} and the metastable
  state, respectively.  The search provides coverage that is
  complementary to existing techniques at the LHC.  }
...
associated to PF candidate particles are corrected to account for
...
itself and pileup events~\cite{Khachatryan:2015iwa,
...
least one charged particle track, and the energy fraction attributed
to charged-hadron particle candidates, $f_{h^{\pm}}$, is required to
be nonzero.
...
0.5. A \scalht-dependent \alphat threshold that decreases from 0.65 at
low \scalht to 0.52 at high \scalht within the range $200 < \scalht <
...
< 5.2$ is also used to efficiently record candidate signal events for
all categories of the SR, including those that satisfy $\njet \geq
1$. The combined performance of these trigger algorithms yields high
...
the matrix-element calculation~\cite{Alwall2014, Kuhn:2005gv},
...
\PYTHIA 8.205~\cite{pythia} program is used to generate diboson (WW,
WZ, ZZ) production.
...
\PYTHIA 8.205~\cite{pythia} program with the \textsc{CUETP8M1}
tune~\cite{Skands:2014pea, Khachatryan:2015pea} is used to describe
parton showering and hadronization for all simulated samples.
...
identify, or veto, well-reconstructed isolated leptons, the parton
density functions~\cite{Butterworth:2015oua}, $\mu_\text{R}$ and
$\mu_\text{F}$, and the modelling of jets from ISR produced in
association with \ttbar~\cite{Chatrchyan:2013xna}. Uncertainties of
100\% in both the NLO QCD and electroweak corrections to the \wj and
\zj simulated samples are also considered. A 5\% uncertainty in the
minimum bias cross section~\cite{Aaboud:2016mmw} is assumed and
...
pileup distributions. Uncertainties in the signal trigger efficiency
...
model processes. Supplemental material is provided to aid further
