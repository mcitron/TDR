...
\abstract{A search for supersymmetry (SUSY) is performed in final
  ...
  prompt decay to quarks and the neutralino.  The masses of bottom,
...
nearby bunch crossings (pileup) are not considered by the PF algorithm
as part of the global event reconstruction. The energy deposits
associated with PF particle candidates are corrected to account for
...
physics object itself and pileup events~\cite{Khachatryan:2015iwa,
...
respectively, and the pseudorapidity requirement $\abs{\eta} < 2.4$.
...
of the energy balance in dijet, multijet, \ggj, and leptonically
...
jets originating from b quarks, as well as the mistag probability to
...
differences in the b tagging efficiency and mistag probability of LF
...
tagging efficiency and mistag probability, as well as corrections for
...
identify, or veto, well-reconstructed isolated leptons, the
PDFs~\cite{Butterworth:2015oua}, $\mu_\text{F}$ and $\mu_\text{R}$,
...
NLO QCD and EW corrections to the \wj and \zj simulated
samples are also considered. A 5\% uncertainty in the total inelastic
...
data and simulation in the pileup distributions. Uncertainties in the
signal trigger efficiency measurements are also propagated to the
transfer factors. The effects of the aforementioned systematic
uncertainties are summarized in Table~\ref{tab:bkgd_systs}, in terms
of representative ranges.  Each source of uncertainty is assumed to
...
effect of W polarization are probed by using \mj events with a
...
b tagging efficiency and mistag probability of LF partons and charm
quarks can be as large as ${\approx}20\%$.
