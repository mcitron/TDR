...
nominal mass of the Z boson~\cite{1674-1137-38-9-090001}. 

For both the muon and dimuon samples, no requirement is made on
\alphat in order to increase the statistical precision of the
predictions from these samples. Both the \mj and \mmj samples are
...
\begin{table}[!tb]
  \topcaption{Threshold values for the final \nb categories used in the 
    search, as a function of \njet and \scalht [GeV]. A dash (--)
    signifies a (\njet, \scalht) category that is not used. The label
    {\it a} signifies the asymmetric $\njet \geq 2$ topology. The
    symbol $\geq$ indicates a final category that covers a range of
    values \nb, bounded by the condition $\nb \leq \njet$ unless the
    \njet category is also unbounded.
  }
  \label{tab:categorisation1}
  \centering
...
200--400, 400--600, 600--900, and 900--1200\GeV, and a final open
bin $\scalht > 1200\GeV$. Only the region $\scalht > 400\GeV$ is
considered for events that satisfy $\njet \geq 4$. Further, the
\scalht binning is adapted per (\njet, \nb) category by removing or
merging bins to satisfy a threshold on the minimum number of data
events in the control regions, which are used to estimate SM
backgrounds, provide checks, and validate assumptions within the
methods. The binning scheme is summarised in
Table~\ref{tab:categorisation}.

Finally, the \mht variable is also used to categorise events as
follows: three bounded bins that satisfy 200--400, 400--600, and
600--900, and a final open bin $\scalht > 900\GeV$. An identical
binning scheme is used across all \nb categories. The \mht variable is
not employed for events with monojet and asymmetric topologies, or
that satisfy $200 < \scalht < 400\GeV$. The maximum value for \mht is
bounded from above for bounded \scalht bins. The lower bound of the
final open \mht bin is limited to that of an unbounded \scalht bin or
900\GeV, whichever is lower.
...
exploiting data sidebands to the signal region, defined by the
requirements $1.25 < \mhtmet < 5.0$ and $0.2 < \bdphi <
0.5$. Estimates from 


Events
are categorised identically to the signal region in terms of \njet and
\scalht. 
