...
Ref.~\cite{Borschensky:2014cia}.

The \PYTHIA 8.2~\cite{pythia} program is used to describe parton
showering and hadronization for all simulated samples. Dedicated
\PYTHIA routines are used to describe the behaviour of long-lived
gluinos and the formation of R-hadrons~\cite{Fairbairn:2006gg,
  Kraan:2004tz, Mackeprang:2006gx}. The description of the detector
response is implemented using the \GEANTfour~\cite{geant} package for
all simulated samples of SM processes or signal models with long-lived
gluinos~\cite{Kraan:2004tz, Mackeprang:2006gx,
  Mackeprang:2009ad}. Scale factors are applied to simulated event
samples that correct for differences in the b-tag efficiency
($\text{SF}_\text{b}$) and mistag probability of light-flavour partons
and charm quarks ($\text{SF}_\text{LF}$) with respect to data, which
typically have values of ${\approx}$0.95--1.00 and
${\approx}$1.00--1.20, respectively, for a jet-\pt range of
40--600\GeV. All remaining signal models rely on the CMS fast
simulation package~\cite{fastsim}, which is consistent with the
\GEANTfour description following the application of corrections of
X--Y and X--Y for differences in the b-tag efficiency and mistag
probability, respectively, as well as corrections of X--Y for
differences in the the modelling of \mht variable. To model the
