...
\newcommand{\ate}{\ensuremath{\mathcal{A} \times \varepsilon}\xspace}
...
\title{Search for natural and split supersymmetric scenarios in \Pp\Pp
  collisions at $\sqrt{s} = 13\TeV$ in all-jet final states}
...
\abstract{A search for supersymmetry is performed in final states
  ...
  prompt decay to SM quarks and the neutralino. 
  % 
  ...
  %Z\GeV are probed.
  %
  The masses of bottom, top, or light-flavour squarks are probed up to
  to X, Y, and Z\GeV, respectively, when produced directly. The gluino
  mass is probed up to to X, Y, and Z\GeV when the gluino decays via
  virtual states of the aforementioned squarks. The strongest mass
  bounds on the daughter neutralinos from gluino or squark decays are
  X and Y\GeV, respectively.
  %
  The search employs standard techniques of event reconstruction, yet
  also provides sensitivity to simplified
  models %inspired by split supersymmetry
  that involve the production and decay of long-lived gluinos.
  ...
  as well as a metastable gluino state. Masses up to
  ${\approx}1.8\TeV$ and ${\approx}1\TeV$ are probed for a proper
  decay length of 1\unit{mm} and the metastable state, respectively.
  %
...
the gauge coupling constants at a high energy~\cite{Dimopoulos:1981yj,
  Ibanez:1981yh, Marciano:1981un}, a dark matter (DM) candidate, and
the gauge hierarchy problem~\cite{ref:hierarchy1,
  ref:hierarchy2}. SUSY particles are expected to be produced in pairs
at the LHC, with heavy states decaying to the lightest stable SUSY
particle (LSP), under the assumption of $R$-parity
conservation~\cite{Farrar:1978xj}. The LSP is assumed to be the
neutralino \PSGczDo, a weakly interacting massive particle and a
viable DM candidate~\cite{Jungman:1995df, 1674-1137-38-9-090001}.
So-called natural models that invoke only a minimal fine-tuning of the
bare Higgs boson mass parameter require only the gluino,
third-generation squarks, and a higgsino-like \PSGczDo at or near the
electroweak scale~\cite{ref:barbierinsusy}. The interest in natural
models is motivated by the discovery of a low-mass Higgs
...
heavy-flavour quarks, accompanied by significant missing transverse
momentum \ptvecmiss.

%%%%%%%%%%

An alternative class of models known as split
supersymmetry~\cite{ArkaniHamed:2004fb, Giudice:2004tc} remove the
requirement of minimal fine tuning of model parameters related to the
Higgs boson mass---in contrast to natural models---yet preserve the
appealling aspects of gauge coupling unification and a DM
candidate. In such a model, only the fermionic superpartners, and a
...
decay is suppressed because of highly virtual squark states. For
...
\PSGczDo. The lifetime of the gluino determines the position of its
decay within the CMS detector, which can lead to final states with
...
luminous region of the proton beams (displaced jets).
...
models that predict the existence of a DM candidate. Signal extraction
is performed using several kinematical variables to provide
sensitivity to a broad range of models. The overwhelmingly dominant
background for a new-physics search in all-jet final states resulting
from \Pp\Pp collisions is the production of multijet events through
the strong interaction and a manifestation of quantum
...
states characterized by the presence of significant \ptvecmiss. The
ATLAS and CMS Collaborations have performed similar searches in
all-jet final states at $\sqrt{s} = 13\TeV$, of which the most
constraining are described in Refs.~\cite{Aaboud:2016zdn,
  Sirunyan:2017cwe, Sirunyan:2017kqq}. The search employs standard
event reconstruction techniques, without the use of specialized
algorithms that target displaced jets. Examples of dedicated
techniques at the LHC can described in Refs.~\cite{}.
...
that require each jet comprises at least two particle candidates, at
least one charged particle track, and a nonzero energy fraction
...
Section~\ref{sec:baseline}, is used as a basis for all data samples used
...
Section~\ref{sec:control}. All selection criteria are summarized in
...
discrimination between multijet events with missing transverse
momentum resulting from instrumental sources, such as jet energy
...
modification to the \bdphi variable is utilized that considers any
...
corresponding bins of the CRs. These adjustments ensure well populated
CRs that are used to estimate the SM backgrounds and validate
...
In total, there are 254 bins in the SR that are defined in
...
are provided for this binning schema with the same likelihood model
...
negligible level and it is estimated using the multijet-enriched
sidebands to the SR defined in Section~\ref{sec:control}. The method is
...
estimated using the \mmj CR defined in Section~\ref{sec:control}
...
CR defined in Section~\ref{sec:control} and the method described in
Section~\ref{sec:ewk}.
...
and contributions from signal models.
...
\PYTHIA 8.2~\cite{pythia} program is used to generate diboson (WW, WZ,
ZZ) production. 

Event samples for signal models involving the production and prompt
decay of gluino or squark pairs, in association with up to two
additional partons, are generated at LO with
{\MADGRAPH{}5\_a\MCATNLO}, and the decay of the SUSY particles is
performed with \PYTHIA 8.2~\cite{pythia}. The \textsc{NNPDF}3.0 LO and
\textsc{NNPDF}3.0 NLO~\cite{nnpdf} parton distribution functions (PDF)
are used, respectively, with the LO and NLO generators described
above.
...
decoupled. Uncertainties in the cross sections are determined from
different choices of PDF sets and factorization and renormalization
scales according to the prescription in
Ref.~\cite{Borschensky:2014cia}. The \PYTHIA 8.2~\cite{pythia} program
...
pair and the \PSGczDo, according to its proper lifetime \ctau [mm]
...
interactions are not exploited by the generic nature of the search.
...
differences in the b-tag efficiency ($\text{SF}_\text{b}$) and mistag
probability of light-flavour partons and charm quarks
($\text{SF}_\text{LF}$) with respect to data. The scale factors have
...
the application of near-unity corrections %of X--Y and X--Y
for differences in the b-tag efficiency and mistag probability,
respectively, as well as corrections %of X--Y
...
\begin{align}
  \tf^{\lost} \, & = \,
  \frac{\mathcal{N}^{\lost}_\text{MC}(\njet, \scalht, \nb, \mht)}
  {\mathcal{N}^{\mj}_\text{MC}(\njet, \scalht, \nb)\hfill} \; ;
  & 
  \mathcal{N}^{\lost}_\text{pred} \, & = \,
  \tf^{\lost} \; \mathcal{N}^{\mj}_\text{data} \; ,
  \\
  \tf^{\znunu} \, & = \,
  \frac{\mathcal{N}^{\znunu}_\text{MC}(\njet, \scalht, \nb, \mht)}
  {\mathcal{N}^{\mmj}_\text{MC}(\njet, \scalht, \nb)\hfill} \; ;
  & 
  \mathcal{N}^\text{\znunu}_\text{pred} \, & = \,
  \tf^{\znunu} \; \mathcal{N}^{\mmj}_\text{data} \; ,
\end{align}

where $\tf^{\lost}$ and $\tf^{\znunu}$ are the transfer factors that
act as multiplier terms on the event counts
$\mathcal{N}^{\mj}_\text{data}$ and
$\mathcal{N}^{\mmj}_\text{data}$ observed in each (\njet, \scalht,
\nb) bin of, respectively, the \mj and \mmj CRs to estimate the \lost
or \znunuj background counts $\mathcal{N}^{\lost}_\text{pred}$ and
$\mathcal{N}^{\znunu}_\text{pred}$ in the corresponding (\njet,
\scalht, \nb, \mht) bins of the SR. The categorization of the \mj and
\mmj event samples is described in Section~\ref{sec:control}. 
...
simulation that are obtained from theoretical calculations or
...
from ISR produced in association with
...
originating from gluons or light-flavour quarks and genuine tags of b
quarks from gluon splitting through the use of loose and tight working
points of the b-tagging algorithm. No tests reveal any evidence of
bias in the \nb modelling.
...
\bdphi < 0.5$. Finally, a ``double'' sideband comprises events that
satisfy all SR selection criteria and the inverted requirements $1.25
< \mhtmet < 3.0$ and $0.2 < \bdphi < 0.5$.
...
\mj and \mmj CRs, following a procedure similar to the one described
in Section~\ref{sec:ewk}.
...
prescriptions described in Section~\ref{sec:ewk}, are also propagated to
...
Sections\ref{sec:control} and \ref{sec:ewk}. The systematic uncertainties
in the nonmultijet estimates are summarized in
...
a ``masked'' fit that uses only the data counts in the \mj and \mmj
...
Section~\ref{sec:categorization}. The result is summarized in
...
mode. Only the gluino-mediated or direct production of top, bottom, or
light-flavour squark pairs are considered, with decays leading to
final states comprising the \PSGczDo and SM particles. The model
parameters are the masses of the parent gluino ($m_\PSg$) and squark
($m_{\PSQ}$, $m_{\PSQb}$, $m_{\PSQt}$) particles, also collectively
labelled as $m_\text{SUSY}$, and the \PSGczDo ($m_{\PSGczDo}$). Two
scenarios are considered for light-flavoured squarks comprising
$\PSQ_\cmsSymbolFace{L}$ and $\PSQ_\cmsSymbolFace{R}$, with $\PSQ =
\{\PSQu, \PSQd, \PSQs, \PSQc\}$: one with an eightfold mass degeneracy
and the other with just a single light squark (\eg
$\PSQu_\cmsSymbolFace{L}$). All other SUSY particles are assumed to be
too heavy to be produced directly. In the case of the production of
gluino pairs, three-body decays via highly virtual squarks are
assumed. In the case of decays via light-flavour squarks, the gluino
is also assumed to be metastable and its lifetime is introduced as a
third model parameter. Lifetimes in the range $10^{-3} < \ctau <
10^{5}\unit{mm}$ are considered. Interpretations are provided for
eight unique model families, as summarized in Table~\ref{tab:sms}.
...
\begin{table}[!t]
  ...
  \begin{tabular}{ lll }
    ...
    & $\Pp\Pp \to \PSg\PSg$,
    $\PSg \to \cPaq\PSQ^* \to \cPaq\cPq\PSGczDo$    
    & $m_{\PSQ} \gg m_{\PSg}$, $10^{-3} < \ctau < 10^{5}\unit{mm}$, stable           \\
    \hline
  \end{tabular}
\end{table}
\endgroup 
...
particles as a function of $m_\text{SUSY}$, $m_{\PSGczDo}$, and
\ctau (if applicable). The approach is based on the aforementioned
...
The experimental acceptance times efficiency (\ate) and is evaluated
independently for each model, defined in terms of $m_\text{SUSY}$,
$m_{\PSGczDo}$, and \ctau (if applicable). Several sources of
uncertainty in \ate are considered. The effects of several sources of
uncertainty in \ate, as well as the potential for migration of events
between bins of the SR, are considered. Correlations are taken into
account where appropriate, including those relevant to signal
contamination that may contribute to counts in the CRs. The magnitude
of each source of uncertainty is dependent on the model and its
parameters.
...
${\approx}30\%$. The \ate for models with small mass splittings (\ie
the regime $m_{\PSQ} - m_{\PSGczDo} \lesssim 200\GeV$) relies somewhat
on jets arising from ISR, the modelling of which is evaluated by
comparing the simulated and measured \pt spectra of the system
recoiling against the ISR jets in \ttbar events, using the technique
described in Ref.~\cite{single-lepton-stop}. The uncertainty can be as
large as ${\approx}30\%$ for models with a compressed mass
spectrum. The corrections to the jet energy scale evaluated with
simulated events can lead to variations in event counts as large as
${\approx}25\%$ for models characterized by high jet multiplicities in
the final state. The uncertainties in the modelling of scale factors
applied to simulated event samples that correct for differences in the
b-tag efficiency ($\text{SF}_\text{b}$) and mistag probability of
light-flavour partons and charm quarks ($\text{SF}_\text{LF}$),
evaluated independently, can be as large as ${\approx}20$ and
${\approx}X\%$, respectively.
...
All families (of production and decay modes) are represented, and the
model parameters ($m_\text{SUSY}$, $m_{\PSGczDo}$, and \ctau if
...
decay lengths. Table~\ref{tab:benchmarks} summarizes the
...
well as variations in the renormalization and factorization scales
...
to one or both gluinos decaying within the calorimeter systems that
...
event vetoes related to the jet identification and
$f_{h^{\pm}}^{\jet{1}}$ requirements, described in
Sections~\ref{sec:reconstruction} and \ref{sec:baseline}, can be as low
as ${\approx}90\%$ and ${\approx}30\%$, respectively. Uncertainties as
large as ${\approx}10\%$ are
assumed. %, motivated in part by studies of the
%simulation modelling of the $f_{h^{\pm}}^{\jet{1}}$ variable in data
%control regions. 
The efficiencies of the aforementioned event vetoes for all other
scenarios are typically ${\approx}100\%$. Jet identification
requirements in the trigger logic lead to inefficiencies and
uncertainties not larger than 2\%. Finally, models with $1 \lesssim
\ctau \lesssim 10\unit{mm}$ often lead to jets that are tagged by the
CSV algorithm with efficiencies as high as ${\approx}60\%$, which are
comparable to the values obtained for jets originating from b
quarks. Uncertainties of 20--50\% in the tagging efficiency are
assumed to cover differences with respect to jets originating from b
quarks, in addition to uncertainties in $\text{SF}_\text{b}$.
...
\begin{figure}[!t]
  ...
  \caption{Observed and expected mass exclusions at 95\% CL
    ...
    the production of gluinos that are assumed to be stable on the
...
mass parameter space for seven families of simplified models. The
...
%in $\alpha_\text{s}$, 
as described in Section~\ref{sec:simulation}. The
former value is determined as a function of $m_{\PSg}$ or $m_{\PSQ}$
and $m_{\PSGczDo}$, while the latter has a dependence solely on
$m_{\PSg}$ or $m_{\PSQ}$. The exclusion of models is evaluated using
observed data counts in the signal region (solid contours) and also
...
mass limits for each family of simplified models.

Figure~\ref{fig:limits-stable} presents the observed
$\sigma_\text{UL}$ as a function of $m_{\PSg}$ and $m_{\PSGczDo}$ for
simplified models that involve the production of gluino pairs. The
excluded mass regions based on the observed and expected values of
$\sigma_\text{UL}$ are also shown, along with contours determined
under variations in theoretical and experimental uncertainties. The
\cmsLeft subfigure shows the sensitivity to models that assume the
prompt decay of each gluino via a virtual light-flavour squark to a
pair of jets and the \PSGczDo. For the \cmsRight subfigure, the gluino
is assumed to be stable and escapes detection, leading to a
monojet-like final state and a reach in $m_\PSg$ that is independent
of $m_{\PSGczDo}$.
%, comparable to former the scenario under a nearly
...
\ctau. The top row of subfigures cover the range $0.001 < \ctau <
0.1\unit{mm}$ and demonstrate coverage comparable to the
\texttt{T1qqqq} prompt-decay scenario. A moderate improvement in
sensitivity for models with $1 \lesssim \ctau \lesssim 10\unit{mm}$ is
observed because of the additional signal-to-background discrimination
provided by the \nb variable. The sensitivity reduces for models with
lifetimes in the region $\ctau > 100\unit{mm}$ because of a reduced
acceptance to the jets from the gluino decay and an increased reliance
on jets from ISR. The coverage is independent of \ctau beyond values
of $10000\unit{mm}$ and comparable to the limiting case of a stable
gluino. The strongest expected and observed mass limits are summarized
...
\begin{table}[!b]
  ...
  \begin{tabular}{ lcc }
    \hline
    Model family                             & \multicolumn{2}{c}{Best mass limit [GeV]} \\
    \cline{2-3}
    ...
    \texttt{T1tttt}                          &                  &                        \\
    \texttt{T1qqqq}                          &                  &                        \\
    \texttt{T1qqqqLL} (Stable $\PSg$)        &                  &                        \\
    \texttt{T1qqqqLL} ($\ctau = 1\unit{mm}$) &                  &                        \\
    \hline
  \end{tabular}
\end{table}
...
momentum of jets. %to provide signal-to-background discrimination.
...
X, Y, and Z\GeV for bottom, top, or light-flavour squarks,
respectively. The corresponding mass bounds on the neutralino \PSGczDo
from squark decays are X, Y, and Z\GeV. The gluino mass is probed up
to to X, Y, and Z\GeV when the gluino decays via virtual states of the
aforementioned squarks. The strongest mass bound on the \PSGczDo from
the gluino decay is X\GeV.
...
particles. Only standard techniques of event reconstruction are
employed. Models that assume a \PSGczDo mass of 100\GeV and gluinos up
to 1.6\TeV are excluded for a proper decay length \ctau below
0.1\unit{mm}. The bound on the gluino mass strengthens to 1.8\TeV at
$\ctau = 1\unit{mm}$, before weakening to ${\approx}1\TeV$ for models
with $\ctau > 10\,000\unit{mm}$ or metastable gluinos. The weakest
exclusion bound on the gluino mass is ${\approx}1\TeV$ for all values
of \ctau considered and for differences in the gluino and \PSGczDo
masses greater than 10\GeV. The search provides coverage that is
complementary to other dedicated techniques at the LHC.
