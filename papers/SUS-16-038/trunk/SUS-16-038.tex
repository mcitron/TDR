...
\title{Search for natural and split supersymmetric scenarios in
  \Pp\Pp\ collisions at $\sqrt{s} = 13\TeV$ in all-jet final states}
...
\abstract{A search for supersymmetry is performed in final states
  ...
  %Z\GeV are probed.
  %
  The masses of bottom, top, and light-flavour squarks are probed up
...
Split supersymmetry~\cite{ArkaniHamed:2004fb, Giudice:2004tc} removes
...
searches in all-jet final states at $\sqrt{s} = 13\TeV$, of which the
most constraining are described in Refs.~\cite{Aaboud:2016zdn,
...
algorithms that target displaced jets. Examples of searches using
specialized techniques at the LHC are described in
Refs.~\cite{Aaboud:2017iio, Aaboud:2016uth, Aaboud:2016dgf,
  Aad:2013gva, Khachatryan:2016sfv, Khachatryan:2015jha}. 
...
displaced jets). The physics object requirements are defined below and
...
SUSY particles is performed with \PYTHIA 8.2~\cite{pythia}. The
...
\PYTHIA 8.2~\cite{pythia} program is used to describe parton showering
...
in any given region of the model parameter space. The statistical
uncertainty arising from the finite size of simulated samples can be
as large as ${\approx}30\%$. The \ate for models with a compressed
mass spectrum relies somewhat on jets arising from ISR, the modelling
of which is evaluated by %comparing the simulated and measured \pt
...
and mistag probability of LF partons and charm quarks, 
%($\text{SF}_\text{LF}$), 
evaluated independently, can both be as large as ${\approx}20$. 
...
1050, 1000, and 1325\GeV for bottom, top, or light-flavour squarks,
respectively. The corresponding mass bounds on the neutralino \PSGczDo
from squark decays are 500, 400, and 575\GeV. The gluino mass is
probed up to 1900, 1650, and 1650\GeV when the gluino decays via
virtual states of the aforementioned squarks. The strongest mass bound
on the \PSGczDo from the gluino decay is 1150\GeV.
...
to 1600\GeV are excluded for a proper decay length \ctau below
