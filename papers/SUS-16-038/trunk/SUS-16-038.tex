...
\hypersetup{ 
  pdfauthor={Robert Bainbridge, Eshwen Bhal, Shane Breeze, Ben
    Krikler, Oliver Buchmueller, Stefano Casasso, Matthew Citron, Adam
...
production of squarks or any generation of gluinos, large or small
...
\begin{table}[!t]
  ...
  \resizebox{\textwidth}{!}{
    \begin{tabular}{ ll }
      ...
      Jet cleaning                      & Veto events containing jets that fail identification criteria or $0.1 < f_{h^{\pm}} < 0.95$    \\ 
...
ZZ) production. Event samples for signal models involving the
production and prompt decay of gluino or squark pairs, in association
with up to two additional partons, are generated at LO with
...
particles other than the \chiz are assumed to be heavy and
decoupled. Uncertainties in the cross sections are determined from
different choices of PDF sets and factorisation and renormalisation
...
describe the formation of R-hadrons through the hadronisation of
gluinos~\cite{Fairbairn:2006gg, Kraan:2004tz, Mackeprang:2006gx}. The
hadronisation process, which is steered according to the default
...
pair and the \chiz, according to its proper lifetime, which is a
parameter of the simplified model. The details of the R-hadron model
are expected to have little influence on the hadronisation of the
quarks originating from the gluino decay (and hence the final state in
terms of event topology and kinematics) for the models considered in
this paper.
%In this study, the interactions of R-hadrons with the detector
%material are not simulated, as this behaviour cannot be exploited by
%the generic nature of this search, unlike more dedicated techniques
%used elsewhere.~\cite{}\fixme{eg reference dE/dx, stopped searches,
...
($\text{SF}_\text{LF}$) with respect to data, which typically have
values of ${\approx}$0.95--1.00 and ${\approx}$1.00--1.20,
respectively, for a jet-\pt range of 40--600\GeV. All remaining signal
models rely on the CMS fast simulation package~\cite{fastsim}, which
is consistent with the \GEANTfour description following the
application of corrections of X--Y and X--Y for differences in the
b-tag efficiency and mistag probability, respectively, as well as
corrections of X--Y for differences in the the modelling of \mht
variable. To model the effects of multiple pp collisions within the
...
mode. Only the $\PSg$-mediated or direct production of top, bottom, or
light-flavour squark pairs are considered, with decays leading to
final states comprising the \chiz and SM particles. The model
parameters are the masses of the parent \PSg\ ($m_\PSg$) and squark
($m_{\PSQ}$, $m_{\PSQb}$, $m_{\PSQt}$) particles, also collectively
labelled as $m_\text{SUSY}$, and the \chiz ($m_\text{LSP}$). Two
...
$\PSg$ pairs, three-body decays via highly virtual squarks are
assumed. In the case of decays via light-flavour squarks, the $\PSg$
is also assumed to be metastable and its lifetime is introduced as a
third model parameter. Lifetimes in the range $3{\times}10^{-6} < \tau
< 3{\times}10^{2}\unit{ns}$ are considered, which correpond to
characteristic decay lengths in the range $10^{-3} < c\tau <
10^{5}\unit{mm}$. Interpretations are provided for eight unique model
families, as summarised in Table~\ref{tab:sms}.
...
As described in Sec.~\ref{}, all jets are subject to identification
and quality requirements. 

The probability for a jet to satisfy identification and quality
requirements varies as a function of its displacement with respect to
the PV, primarily due to 

identification and quality
requirements 

arising from the decay of long-lived gluinos 

efficiency jets with displaced vertices

Finally, 
resulting from the decay of long-lived $\PSg$ particles, as well as
the probability of tagging these displaced jets as arising from b
quarks, are inspected as a function of the displacement. BLAH
... Additional uncertainties associated with these quantities are
assigned BLAH ...

%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Figure~\ref{fig:limits} summarises the excluded regions of the mass
parameter space for seven families of simplified model. The regions
are determined by comparing the upper limits on the measured fiducial
cross section, corrected for the experimental
$\mathcal{A}\,\varepsilon$, with the theoretical cross sections
calculated at NLO+NLL accuracy in $\alpha_\mathrm{s}$, as described in
Sec.~\ref{sec:simulation}. The former value is determined as a
function of $m_{\PSg}$ or $m_{\PSQ}$ and $m_{\PSGczDo}$, while the
latter has a dependence solely on $m_{\PSg}$ or $m_{\PSQ}$. The
exclusion of models is evaluated using observed data counts in the
