...
\abstract{A search for supersymmetry (SUSY) is performed in final
  ...
  prompt decay to quarks and the lightest neutralino.  The masses of bottom,
  top, and mass-degenerate light-flavour squarks are probed up to
  1050, 1000, and 1325\GeV, respectively. The gluino mass is probed up
  to 1900, 1650, and 1650\GeV when the gluino decays via virtual
  states of the aforementioned squarks. The strongest mass bounds on
  the daughter neutralinos from gluino and squark decays are 1150 and
  ...
  and decay of long-lived gluinos.  Proper decay lengths from
  $10^{-3}$ to $10^{5}\unit{mm}$ are considered, as well as a
  metastable gluino scenario. Gluino masses up to 1750 and 900\GeV are
  probed for a proper decay length of 1\unit{mm} and for the
  metastable state, respectively.  The search provides coverage that
  is complementary to existing techniques at the LHC.  }
...
the CERN LHC and to decay to the lightest stable SUSY particle
...
jets with particle vertices located a significant distance (\ie
...
angle and pseudorapidity. Isolation for an electron or muon is a
...
charged particle track, and the energy fraction attributed to
charged-hadron particle candidates, $f_{\text{h}^{\pm}}$, is required
to be nonzero.
...
model~\cite{Buchmueller:2017uqu}. Values within the range $10^{-3} <
\ctau < 10^{-3}\unit{mm}$ are considered. Studies with alternative
...
the effects of multiple pp collisions within the same or neighbouring
...
of variations related to: the jet energy scale, the scale factors
...
$m_{\PSQt}$, $m_{\PSQ}$) squark, also collectively labelled as
...
families, as shown in Fig.~\ref{fig:limits-sms} (\cmsRight), can be up
...
discrimination provided by the \nb variable. The sensitivity reduces
for models with lifetimes in the region $\ctau > 100\unit{mm}$ because
of a reduced acceptance for the jets from the gluino decay and an
