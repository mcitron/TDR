...
if rebin :
    upper.rebin()
upper.h_.GetXaxis().SetRangeUser(0.3,3.1)
upper.draw()
...
partial.draw("samehist")

if 1 :
    residual.draw("samehist")
else :
    residual.draw("samehist")
    residual1.draw("samehist")
    residual2.draw("samehist")
...
for sample,title,opts in [(data.h_,"Data","p"),
                          (sm.h_,"SM total","lf"),
                          (total.h_,"W#rightarrowl#nu+jets","f"),
                          (partial.h_,"t#bar{t}+jets","f"),
                          (residual.h_,"Residual SM","f"),
#                          (residual.h_,"Top, VV, Z#rightarrowll + jets","f"),
                          ] :
    print sample,title
...
if rebin :
    lower.rebin()
lower.h_.GetXaxis().SetRangeUser(0.3,3.1)
lower.draw()

line = r.TLine(0.3,1.,3.5,1.)
line.SetLineWidth(1)
line.SetLineStyle(2)
