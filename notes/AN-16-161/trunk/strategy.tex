...
\item improvements concerning simplified Dark Matter models and compressed SUSY
  models. For example adding the monojet and asymmetric jet categories.
...
\subsection{Recent updates and outstanding issues with inputs\label{sec:disclaimers}}
%Several updates in terms of datasets and MC corrections have recently been implemented. 
%Few of them are still missing, but they are expected to come in the next days, in time for the release of the 
%new version of the documentation before the approval of the analysis. 
...
\item \textbf{Data/MC correction factors}. The following data/MC scale factors (SF) have been used in this iteration of the analysis. 
  ...
  \item b-tagging scale factors: the official set of scale factors corresponding
    to the ICHEP dataset have been applied according to the recommendations from
    the BTV POG \cite{btagSFMethods2016}. Both FullSim and FastSim MC samples are
    reweighted with their corresponding scale factors and correction factors. We 
    have now also separated the effects of tagging efficiencies and mis-tagging
    rates into separate nuisances.
  \item Muon identification/isolation corrections: the latest
    corrections are used, taken from the Muon working group twiki
    \cite{twiki-leptonSFMuonPOG2016} and the SUSY lepton scale factors
    working group \cite{twiki-leptonSFSUSY2016}.
  ...
  \item Photon identification: the photon ID scale factors derived by the
    e-gamma POG using the full ICHEP dataset have been applied.
  ...
  \item Weighting of simulated events according the generator-level top \Pt is performed.
...
%    the other (un-corrected) background samples, like \wj or \zj, when the top-pt rewighting is included. 
  \end{itemize}
\item Signal models: the latest recommendations from the SUSY PAG for the
  treatment of signal systematics (including those related to ISR) and for
  the limit setting, have been adopted \cite{twiki-SUSYICHEP16}.
\end{itemize}
