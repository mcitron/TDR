...
%% The background predictions in the signal region are extracted through a combined maximum likelihood fit 
%% to the observations in the control regions. The details of the likelihood model are highlighted in Sec.~\ref{sec:likelihood}. 
%% In Appendix~\ref{app:} the predictions from simulation are also presented.
...
The predicted yields are based on a combined fit to the data control samples considering every (\nj,\nb,\scalht) category. 
The observed counts in the signal region are not considered in this fit. 
The uncertainties reflect all statistical and (pre-fit) systematic sources with their correlations. 
...
In Fig.~\ref{fig:summaryPlot_Monojet}-~\ref{fig:summaryPlot_Symmetric} the background yield predictions 
...
including both the statistical and systematics components. Pulls are shown both for the control region only fit (CR-only) and the full fit. 
...
observed yields in the signal region corresponding to an integrated luminosity of 2.6 \ifb. 
