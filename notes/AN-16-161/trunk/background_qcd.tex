...
\subsubsection{Reduction in QCD prediction from the 2015 analysis}

Since the QCD prediction was carried out for the 2015 analysis, the
total number of events predicted in the signal region has consistently
reduced across all analysis bins. The major difference that has led to
this change is an update in the QCD simulation that has been used.
...
version of CMSSW. From the 74X to the 80X MC there was a significant
change in the modelling of the \mhtmet distribution. This is
demonstrated in Fig.~\ref{fig:mhtDivMetChange}. In the 80X QCD MC
significantly fewer events pass the \mhtmet cut than in the 74X MC. 

As a large proportion of the QCD MC prediction is made from a few events
with very high weights, there is always a large statistical
...
despite the 2016 application of the method resulting in lower values,
the prediction in each bin is still statistically compatible with the
2015 prediction. 

In the parked data analysis of 8\tev data the level of QCD
contamination predicted in the analysis was much lower than that seen
in the 2015 data. With the change in the QCD simulation, we now
observe numbers that are more compatible with what was seen in Run~1.
...
\begin{figure}[!h]
  ...
  \subfigure[\mhtmet distributions in the control region with 74X QCD
  MC.]{
    \includegraphics[width=0.5\textwidth]{figures/qcd/mhtDivMet/cr74X}
  } \\
  ...
  \subfigure[\mhtmet distributions in the control region with 80X QCD
  MC.]{
    \includegraphics[width=0.5\textwidth]{figures/qcd/mhtDivMet/cr80X}
  } \\
  ...
  between the MC reconstructed with CMSSW 74X and 80X.}
