...
\begin{table}[h!]
  ...
  \begin{tabular}{ ll }
    ...
    Odd jet veto                  & Veto events containing at least one jet within acceptance that fails Loose ID                              \\
...
Events are vetoed if any jet satisfies the aforementioned \pt and
$\eta$ acceptance requirements yet fails the Loose ID working point
described in Tab.~\ref{tab:loose-jet-id} (Sec.~\ref{sec:jetreco}). 
...
\begin{table}[!h]
  \topcaption{Summary of the event selection criteria for the control
    regions. The criteria are applied in addition to the pre-selection
...
\begin{table}[!t]
%  \topcaption{Summary of the \HTmiss binning scheme as a function of
%    \njet, \nb, and \scalht [GeV]. The \HTmiss values signify the
%    lower bound of each bin. Each bin is bounded from above by the
%    following one, except for the final open \HTmiss bin, as indicated
%    by the $>$ symbol. A dash (--) signifies that the (\njet, \nb
%    \scalht) category is not used in the analysis. } 
  \topcaption{Summary of the \HTmiss binning scheme as a function of
    \njet, \nb, and \scalht [GeV]. The \HTmiss values signify the
    lower bound of each bin. Each bin is bounded from above by the
    following one, except for the final entry, which corresponds to
    the final open \HTmiss bin indicated by the $>$ symbol. A dash
...
The binning strategy employed in the signal and control regions can be
