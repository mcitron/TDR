...
categories. A goodness-of-fit estimate can be obtained by considering
the pulls obtained from the masked fit, which in the following are
assumed to be derived from independent background estimates. The
summation over the squares of the pulls from 101 (\njet, \nb, \scalht)
categories, summarised in Fig.~\ref{fig:pulls}, gives a $\chi^2$ value
of 73.0. Hence the reduced $\chi^2$ and the p-value are 0.72 and 0.98,
respectively.
...
\nb) event category and \scalht bin. Appendix~\ref{app:nuispost} also
contains figures showing the post-fit behaviour of the nuisance
parameters relative to their pre-fit definitions.
