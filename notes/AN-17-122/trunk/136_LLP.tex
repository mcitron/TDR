...
Table~\ref{tab:benchmark-LLP} summarises a set of benchmark models
studied in this section, each of which is defined in terms of the
masses of the gluino and LSP, as well as $c\tau$. Also listed is a
representative event topology, defined in terms of \njet and \nb, that
plays an important role in providing sensitivity to each model.
...
\begin{figure}[!ht]
    ...
    \subfigure[$\ctau=100\unit{mm}$, compressed $(1000,900)$]{
    ...
    \caption{ Signal shapes for the \texttt{T1qqqqLL}
      $c\tau=10~\mu\mathrm{m}$ (upper), $c\tau=1~\mathrm{mm}$ (middle),
      and $c\tau=100~\mathrm{m}$ (lower) benchmark models with
...
criteria used to define the signal region. The final value in each
table represents the efficiency for the sensitive topological (\njet,
\nb) region defined in Table~\ref{tab:benchmark-LLP}.
