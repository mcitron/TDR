...
The transfer factors account for differences between the \mj control
and signal due to the acceptance times efficiency for the
reconstructed muon and extrapolations in kinematic quantities. For
example, the dependence on \njet, \scalht, and \nb is largely
attributable to the \alphat and \bdphi requirements for the signal
region (that are not used in the \mj control region). 
...
\HTmiss modelling (Sec.~\ref{sec:}). Further, the uncertainty in these
corrections are not propagated, as any inaccuracy is -- by
construction -- covered by the aforementioned data-driven procedures
to determine systematic uncertainties.
...
A floating parameter per process (\wj ) encodes the correction for
that process (fully correlated across all bins).  The \wj and \ttbar
processes are mainly constrained by the \mj sideband while the \zj
process is constrained by the \mmj sideband. The values of the
...
\subsubsection{PDF and scale uncertainties}
\label{sec:tfSyst_pdf}

Theoretical uncertainties from choices in the parton distribution
function and renormalisation/factorisation scale can introduce
systematic uncertaintes on lepton acceptance, \ie the fraction of
leptons out of $p_{T}$ or $\eta$ range. The effect can then be
parametrized with the variable \flepAccep in
Section~\ref{sec:leptonSyst}. Theoretical uncertainties from parton
distribution function are varied according to the recommended
prescription from PDF4LHC~\cite{PDF4LHC:2015}. The scales $\mu_R$ and
$\mu_F$ are varied by 0.5 and 2.0. The systematic variations are found
to be less than 1\%.

\subsubsection{\texorpdfstring{\wj}{W+jets} and \texorpdfstring{\ttbar}{TTbar} composition}
\label{sec:tfSyst_ttW_composition}

The normalisation of the simulated \wj and \ttbar event samples are
corrected to data in the \HTmiss sideband using the procedure
described in Sec.~\ref{sec:sideband-corrections}. 

Subsequently, the normalisation for the \wj process is varied by twice
the relative uncertainty in the latest CMS measurement of the
inclusive cross section, while preserving the sum of the \wj and
\ttbar contributions, to see the effect on the ``shape'' of the
transfer factors and the \HTmiss templates. Similarly, the same
same procedure is repeated for variations in the \ttbar
normalisation. The resulting variations are summarised in
