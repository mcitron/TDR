...
with the exception of 5.2~\ifb of the certified data in Run2016G. 
Data in the control regions is never blinded. 
These blinding policies are agree in the SUS PAG.

\begin{table}[!h]
\topcaption{The JSON file that specifies the periods of the time in
which 12.9~\ifb of the certified data are collected} \footnotesize
...
It can be noticed that the simulation does not contain events with 37 or more overlapping 
pp interactions, while the data does, causing the MC weighted distribution not to match perfectly the data. 
This small issue will be recovered once the new MC samples with updated PU profile will be available. 
...
Section \ref{sec:closure-tests}. 
As described in Section~\ref{sec:sideband_corrections}, residual cross section
