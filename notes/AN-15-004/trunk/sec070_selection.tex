...
are required to satisfy $\PT>100\gev$.% --this isn't required currently, it may be in the future--- and the leading jet must be within $|\eta|<2.5$. 
...
\begin{table}[h!]
\topcaption{Preliminary signal region binning \label{tab:alphat-binning}}
  \centering
  \footnotesize
  \begin{tabular}{ lcccccc }
    \hline
...
Also a new orthogonal signal region has been added. In order to increase acceptance and sensitivity for ISR and mono-jet type events is lowered to 40~GeV with an upper bound of 100~GeV, see Sec~\ref{sec:} \textbf{FIXME: to be checked where this description goes}. Bins for this new signal region are given in Tab.~\ref{tab:alphat-asym-binning}.


\begin{table}[h!]
\topcaption{Preliminary asymmetric signal region binning \label{tab:alphat-asym-binning}}
  \centering
  \footnotesize
  \begin{tabular}{ lcccccc }
    \hline
...
As described in Sec.~\ref{sec:controlSelection}, the
...
% --- this next bit was found to not really be true due to the high pT requirements anyway
