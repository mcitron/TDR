...
least five jets, plus exactly zero, one, two, or at least three
b-jets. By construction, $\nb \leq \njet$.

{\bf \HT requirements and default binning.} 
...
bins in the range $200 < \HT < 400\gev$ and 200\gev bins in the range
$\HT > 400\gev$. The final \HT bin is inclusive.

{\bf Event categorisation versus integrated luminosity.} 
...
{\bf \HT binning versus integrated luminosity.} 

The lower threshold of the last (inclusive) \HT bin is determined
independently for each event category and is chosen to always align
with one of the ``default'' boundaries defined above. The metric for
choosing the final bin threshold is based on the number of events in
the corresponding \HT bin (and event category) of the data control
samples. This ensures that all bins in the data control samples are
sufficiently populated to ensure a statistically significant
prediction in each of the corresponding signal region bins. Currently,
sufficient events ($\sim 10$) are required such that the statistical
uncertainty on the total prediction is $\sim30\%$. Events from high
\HT bins are combined into a single inclusive bin that satisfies this
metric.

{\bf Final common categorisation and binning choice for all samples.} 
...
\begin{table}[h!]
  \caption{Threshold (GeV) of the final \HT bin as a function of event
    category (\njet,\nb). The threshold is always aligned with respect
    to one of the default boundaries (motivated primarily by the trigger) of
    200, 250, 300, 350, 400, 600, 800, 1000\gev. This is the projected
    choice for 3\fbinv.}
  ...
  \begin{tabular}{ l|ccc|cccc|cccc|cccc }
    ...
    \hline
    Symmtric   & 800 & 800 & 600 & 800 & 800 & 800 & 400 & 800 & 800 & 800 & 600     & 800 & 800 & 800 & 800     \\
...
Two additional dedicated cleaning cuts are applied.

First, to protect against multiple jets failing the $\Et$ threshold,
the jet-based estimate of the missing transverse energy, \mht, is
compared to the missing transverse energy variable, $\met$, and events
with $R_{\rm miss}=\mht/\met > 1.25$ are rejected. 

Second, to protect against severe energy losses caused by masked
regions in the ECAL (which amount to about 1\% of the ECAL channel
count) or HCAL, or by missing instrumentation in the barrel-endcap
gap, events with $\bdphi < 0.5$ are rejected if the distance in the
($\eta,\phi$) plane between the selected jet and the closest masked
ECAL region, $\Delta R_{\rm ECAL}$, is smaller than 0.3. Similarly,
events are rejected if the jet points within 0.3 in $\eta$ of the ECAL
...
\begin{table}[h!]
\topcaption{Summary of the signal region definition\label{tab:sr-selections}}
  \centering
  \footnotesize
  \begin{tabular}{ lrl }
    \hline
    \hline
    \multicolumn{2}{c}{Selection} & Requirement                                                                      \\
    \hline
    ``MET filters''               & (pre-selection) & Various                                                        \\
    Jet acceptance                & (pre-selection) & $\PT > 40\gev$, $|\eta| < 3$                                   \\
    \njet                         & (pre-selection) & $\geq2$                                                        \\
    Lead jet acceptance           & (pre-selection) & $\PT > 100\gev$, $|\eta| <    2.5$                             \\
    Second jet acceptance         & (pre-selection) & $\PT > 100\gev$ \texttt{OR} $40 < \PT < 100\gev$               \\
    \HT                           & (pre-selection) & $\HT > 200\gev$                                                \\
    \hline
    \alphat                       & (signal region) & $>$0.52--0.65 (\HT-dependent) for region $200 < \HT < 800\gev$ \\
    \mht                          & (signal region) & $>130\gev$ for region $\HT > 800\gev$                          \\  
    \bdphi                        & (signal region) & $>0.3$                                                         \\
    \mht/\met                     & (signal region) & $<1.25$                                                        \\
    ``Dead ECAL filter''          & (signal region) & (see text)                                                     \\
    \hline
    \hline
  \end{tabular}
\end{table}
...
the \hg sample can only be used to predict background components in
