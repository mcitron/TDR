...
\item general optimisations concerning trigger, event reconstruction and
  selection;
\item improvements concerning simplifed Dark Matter models and compressed SUSY
  models. 
...
jet categories are presented for the first time. These addtitions consitute a significant effort and aim to achieve very good 
...
To improve acceptance particularly for low WIMP and mediator masses in simplified
SUSY and DM models and ensure best possible acceptance we also lowered the jet multiplicity 
requirement of the analysis to include the 'exactly one jet' or 'monojet' bin. The jet selection criteria, 
in particular the \Pt threshold remain unchanged at $\Pt>100$~GeV.
Because the \alphat~variable is not defined in this case we use the $\bdphi > 0.5$ requirement to remove multijet
background.
...
background and the $W$ charge asymmetry. Also the addition of two electron control samples
will allow for additional closure tests, further improving the understanding of the transfer factors constructed
...
%the \mht variable also is employed.
