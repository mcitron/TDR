...
\begin{figure}[h!]
  \begin{center}
    \subfigure[Legend]{\includegraphics[width=0.3\textwidth]{figures/closureTests/legend.png}} \\
...
In order to inspect the effect of a significant systematic 
on the \mht shape of the background, a ``toy model'' for alternative shapes, corresponding 
to a hypothetical variation of a source of uncertainty, has been developed.
The alternative histogram templates are obtained from the nominal one by 
scaling the content of each bin with an exponential factor, whose parameters 
are determined requiring a plus/minus 50\% shift in the highest \mht bin for the up/down variation respectively. 
The resulting alternative shapes are shown, for one category and \HT bin as an example, 
in figure \ref{fig:mht-shape-syst-toy}.
