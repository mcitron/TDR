...
two sets. Additionally, a few more are included specific to each of
the background processes.

In Fig.~\ref{fig:ttWClosure} the closure tests selected for the W and
\ttbar backgrounds are presented. The exact closure tests are detailed
...
the single electron control sample as well as the muon control sample.
...
Sec.~\ref{sec:closure-tests-desc}. This is most likely due to the fact
that the calculated systematic is a statistical statement about the
extent to which we can say a particular bin is free of bias. As the
closure tests selected for W and \ttbar are now only based on the higher
statistic single lepton control samples there is a better level of
control, resulting in a smaller expected systematic. In most bins the
...
and low \scalht bins are the most effected, but as the \znunu
