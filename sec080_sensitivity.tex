...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\subsubsection{\texorpdfstring{\HT}{HT} evolution models}
...
\begin{table}[h!]
  ...
  \begin{tabular}{ llcc }
    ...
    \hline
    \texttt{T1bbbb}           & \Ttwobbbb               & (4,2),(4,3),(4,4),($\geq 5$,2), ($\geq 5$,3),($\geq 5$,$\geq 4$) \\ % (2--3,1), 
...
uncertainties on the backgrounds as $0.10$ (\htscale 200-600), $0.20$ (\htscale 600-1000),
$0.30$ (1000-$\inf$). The \htscale bins used were motivated as detailed in ???. Using the 
likelihood model from \ref{sec:hadronicLikelihood} and the \mj control sample (as $\nb > 1$) 
a limit was found for the T1bbbb model for luminosities of 1 and 4 fb. This is shown along 
with the theoretical cross section in \ref{sec:limit}. At 4fb it can be seen that we exclude to 1 million sigma.

\begin{figure}[h!]
  \begin{center}
      \includegraphics[width=0.45\textwidth, trim=0 190 290 20, clip=true]{figures/} % lbrt
    \label{fig:limit}
  \end{center}
\end{figure}
